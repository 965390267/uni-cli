<!-- 
 ...
 -->
<template>
	<view class="org-image-base" :style="orgStyle">
		<view class="org-image " :style="styles" :class="{'error-max-size':showDefault||!src}">
			<view v-if="showDefault||!src" :style="styles">
				<image mode="aspectFit" class="error-default" :class="{'rotate-animation':showAnimation}" :src="defautlError"
				 :style="styles"></image>
			</view>
			<template v-else>
				<image :fade-show="fadeShow" :mode="mode" :src="imgSrc" lazy-load="true" :style="styles" @error="errorImage(imgSrc)"
...
<script>
	...
	export default {
		...
		created() {
			...
			this.css['min-height'] = this.size.height;
